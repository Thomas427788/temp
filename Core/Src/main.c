...
/* USER CODE BEGIN PV */
osEventFlagsId_t g_cmd_event;
CommandParams    g_command_params = {0};
volatile uint8_t g_command_received = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_TIM2_Init();
  /* USER CODE BEGIN 2 */
  // Nothing needed here anymore - task handles everything
  /* USER CODE END 2 */
  ...
  LoadCellTaskHandle = osThreadNew(StartLoadCellTask, NULL, &LoadCellTask_attributes);
  ...
  g_cmd_event = osEventFlagsNew(NULL);
...
__weak void StartPrintTask(void *argument)
{
  ...
  #if 0
	  printf("FreeRTOS + USB CDC ready!\r\n");

	  for (;;)
	  {
		printf("Hello from FreeRTOS! Tick: %lu\r\n", osKernelGetTickCount());
		osDelay(1000); // Always use osDelay in FreeRTOS tasks, never HAL_Delay
	  }
  #endif
...
__weak void StartLoadCellTask(void *argument)
{
...
__weak void StartStepperTask(void *argument)
{
